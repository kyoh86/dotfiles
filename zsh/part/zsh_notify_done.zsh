...
if [ -n "${NVIM_SERVER_NAME}" ] ; then
  function _notify_precmd_to_nvim() {
    nvim --server ${NVIM_SERVER_NAME} --headless --remote-send "<cmd>doautocmd User Kyoh86TermNotifReceived:precmd:$?:$KYOH86_VOLATERM_BUFNR:$(echo $history[$(($HISTCMD-1))] | base64 -w 0)<cr>"
  }
