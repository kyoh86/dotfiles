...
function M.run(command, opts)
  ...
  if #command > 0 then
    if opts.quit == "wait" then
      command = vim.list_extend(command, { ";", "echo", vim.fn.shellescape("Press Enter key to quit"), ";", "read" })
    end
  ...
  local result = vim.system({ "tmux", "split-window", opts.split == "horizontal" and "-h" or "-v", "-b", shellCommand }):wait()
  return result.code
end
